  ...
  run() {
    ...
    function withAuthor(callback) {
      return function (id, created, room, user, body) {
        bot.User(user, (_err, userData) => {
          if (userData.user) {
            const author = self.robot.brain.userForId(
              userData.user.id,
              userData.user
            )
            const userId = userData.user.id
            self.robot.brain.data.users[userId].name = userData.user.name
            self.robot.brain.data.users[userId].email_address =
    ...
    bot.Me((_err, data) => {
      ...
      return Array.from(bot.rooms).map((roomId) =>
        ((roomId) =>
          bot.Room(roomId).join((_err, callback) => bot.Room(roomId).listen()))(
          roomId
        )
      )
    })
...
class CampfireStreaming extends EventEmitter {
  constructor(options, robot) {
    ...
    this.authorization = `Basic ${Buffer.from(`${this.token}:x`).toString(
      "base64"
    )}`
    this.private = {}
  }
    ...
    return {
      ...
      listen() {
        const headers = {
          ...
          "User-Agent": `Hubot/${
            this.robot != null ? this.robot.version : undefined
          } (${this.robot != null ? this.robot.name : undefined})`,
        }
