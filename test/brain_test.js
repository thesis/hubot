  ...
  describe("Unit Tests", () => {
    describe("#mergeData", () => {
      ...
      it("emits a loaded event with the new data", function () {
        ...
        expect(this.brain.emit).to.have.been.calledWith(
          "loaded",
          this.brain.data
        )
      })
    ...
    describe("#usersForRawFuzzyName", () => {
      it("does a case-insensitive match", function () {
        expect(this.brain.usersForRawFuzzyName("guy")).to.have.members([
          this.user1,
          this.user2,
        ])
      })
      ...
      it("returns all matching users (prefix match) when there is an exact match (case-insensitive)", function () {
        // Matched case
        expect(this.brain.usersForRawFuzzyName("Guy One")).to.deep.equal([
          this.user1,
          this.user2,
        ])
        // Mismatched case
    ...
    describe("#usersForFuzzyName", () => {
      it("does a case-insensitive match", function () {
        expect(this.brain.usersForFuzzyName("guy")).to.have.members([
          this.user1,
          this.user2,
        ])
      })
      ...
      it("returns just the user when there is an exact match (case-insensitive)", function () {
        // Matched case
        expect(this.brain.usersForFuzzyName("Guy One")).to.deep.equal([
          this.user1,
        ])
        // Mismatched case
        expect(this.brain.usersForFuzzyName("guy one")).to.deep.equal([
