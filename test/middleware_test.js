  ...
  describe("Unit Tests", () => {
    ...
    describe("#register", () => {
      ...
      it("validates the arity of middleware", function () {
        ...
        expect(() => this.middleware.register(testMiddleware)).to.throw(
  ...
  describe("Public Middleware APIs", () => {
    beforeEach(function () {
      ...
      this.robot.on("error", (name, err, response) => {
        if (
          __guard__(
            err != null ? err.constructor : undefined,
            (x) => x.name
          ) === "AssertionError"
        ) {
          process.nextTick(() => {
        ...
        it("is the listener object that matched", function (testDone) {
          this.robot.receive(this.testMessage, () => {
            expect(this.middleware).to.have.been.calledWithMatch(
              sinon.match.has("listener", sinon.match.same(this.testListener)), // context
        ...
        it("has options.id (metadata)", function (testDone) {
          this.robot.receive(this.testMessage, () => {
            expect(this.middleware).to.have.been.calledWithMatch(
              sinon.match.has(
                "listener",
                sinon.match.has("options", sinon.match.has("id"))
              ), // context
              sinon.match.any, // next
              sinon.match.any // done
            )
