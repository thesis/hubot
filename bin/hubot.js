...
function loadHubotScripts() {
  ...
  if (fs.existsSync(hubotScripts)) {
    ...
    try {
      ...
      const err = error;
      robot.logger.error(
        `Error parsing JSON data from hubot-scripts.json: ${err}`
      );
      process.exit(1);
    }

    hubotScriptsWarning =
      "Loading scripts from hubot-scripts.json is deprecated and " +
    ...
    const hubotScriptsReplacements = pathResolve(
      "node_modules",
      "hubot-scripts",
      "replacements.json"
    );
    ...
    if (!fs.existsSync(hubotScriptsReplacements)) {
      hubotScriptsWarning +=
        "To get a list of recommended replacements, update your hubot-scripts: npm install --save hubot-scripts@latest";
      return robot.logger.warning(hubotScriptsWarning);
    }
    ...
    if (scriptsWithoutReplacements.length > 0) {
      ...
      hubotScriptsWarning += scriptsWithoutReplacements
        .map((script) => `* ${script}\n`)
        .join("");
      hubotScriptsWarning +=
